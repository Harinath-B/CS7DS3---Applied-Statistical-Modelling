...
## i
As stated in the **Probability Review II** notes under the section **Expectation and variance of random variables**,
...
## ii
This was done via R code using the pbinom() function. The value $P(5 <= A <= 10)$ was computed as $P(A <= 10) - P(A <= 4)$
...
## ii
Under the Beta-Binomial model, for the prior $Be(a, b)$ the posterior can be constructed (with $k = \Sigma x_i$) as $Be(a_n, b_n)$ where $a_n = k + a$; $b_n = n - k + b$
    $$a_n = k + a = 3 + 3$$
    $$a_n = 6$$
    $$b_n = n - k + b = 25 - 3 + 30$$
    $$b_n = 52$$
...
- The prior was strong, but not strong as the observed data was still able to influence the posterior in a non-insignificant way.
...
- The posterior of $\theta_2$ stretches the distribution distribution resulting in a increase of mean $\theta$ ($0.195 > 0.104$), the variance is higher ($0.0049 > 0.0016$) as the values are more spread out.
...
# Q5
## i
The probability of failure of a $S_3$ system is given by the posterior probability $P(\theta_3|x)$. The 1000 $S_3$ systems can be modeled as a binomial distribution with $n = 1000$ and probability $P(\theta_3|x)$. 
...
- The Expected price can be computed as the average price of the samples
    $$E[Price] = \frac{1}{N} \sum_{i=1}^{N} Price_i$$
    and is computed as `sum(mc_post_payments) / length((mc_post_payments))` and the result expected payments is (approx) **924.81**
...
- The Expected price is computed the same way as in Q5(i) and the result expected payments is **656.76**
...
- Using a non-informative prior significantly increases the chances of failure and consequently recudes the expected price
...
The likelihood function is,
    $$L =  \binom{n}{k}\theta_1^k(1-\theta_1)^{n-k}$$
Taking the log likelihood,
    $$l = log (L) = log\ \binom{n}{k} + log\  \theta^{2k} + log\ (1-\theta^2)^{n-k}$$
The term $log\ \binom{n}{k}$ is a constant with respect to $\theta$ and can be ignored,
    $$l = 2k\ log(\theta) + (n - k)\ log(1 - \theta^2)$$
